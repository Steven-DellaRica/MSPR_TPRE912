...
def generate_strong_password(length: int = PASSWORD_LENGTH) -> str:
    print("DEBUG: entrée dans generate_strong_password", file=sys.stderr)
...
def store_password_to_db(user_id: str, password: str) -> None:
    print("DEBUG: entrée dans strong_password_to_db", file=sys.stderr)
...
def generate_qr_code(data: str) -> str:
    print("DEBUG: entrée dans generate_qr_code", file=sys.stderr)
...
def handle(event, context):
    ...
    try:
        req = json.loads(event)  # Parse le body brut
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "body": {"error": f"Échec stockage : {str(e)}"}
        }
